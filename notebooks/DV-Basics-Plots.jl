...
PlutoUI.TableOfContents(title = "Plots and StatsPlots")
...
# ╔═╡ 2732d1de-9a9c-4f1c-aebe-524a9ce7061a
plotly()
...
# ╔═╡ 57c65392-9235-4446-82d3-49f45f2e9b1c
plot(regions_cum.Region, regions_cum.Pop2019, seriestype = :bar)

# ╔═╡ 86dd3722-49e6-4b89-bf34-6b9ae1bac13a
md"""
Here we can see, that `Plots` by itself is a very basic plotting package (not comparable to `Gadfly` or `VegaLite` for statistical plotting).

Only the additional layer provided by `StatsPlots` brings the required functionality. Among other things it's possible
- using `DataFrames` directly using the `@df`-macro
- to group the data using the `group`-attribute (which gives a result similar to  Gadfly or VegaLite, but differs conceptually because it doesn't use the graphical dimension 'color' in this context)
- to render different diagram types using convenience functions like `bar`, `scatter` etc. (but it's still possible to use `plot` with the `seriestype`-attribute)
And as the following diagram shows, it has a more "intelligent" mechanism for generating useful legends.
"""

# ╔═╡ d64f1f97-e2a8-4d54-bef8-dda7f4d5deaf
@df regions_cum bar(:Region, :Pop2019, group = :Region)

# ╔═╡ c5f6de90-8e8f-11eb-3956-2fc2c986a88b
md"""
The next version uses serveral formatting options (from the `Plots`-package). It has different labels (x-axis, y-axis, title) and a nice background colour.
"""

# ╔═╡ 0d08d9b5-fb06-49b5-b90d-4b33409e157e
@df regions_cum bar(:Region, :Pop2019, group = :Region,
	xlabel = "Region", 
...
# ╔═╡ b5b8db8f-2c0b-4021-bb8e-b4952994552d
@df subregions_cum bar(:Subregion, :Pop2019, group = :Region)

# ╔═╡ a3d9c720-8e92-11eb-388d-cf787ea1d206
md"""
As there are quite a few subregions, a horizontal bar diagram might be more readable. Apart from that we have to specify `yticks = :all` and give the whole diagram more space via `size = (600, 600)` so that Plots shows all lables on the y-axis.
"""

# ╔═╡ 2aaacd5e-0e7f-4696-aeaf-d7382a2b29ab
@df subregions_cum bar(:Subregion, :Pop2019, group = :Region,
	orientation = :hor,
...
md"""
Trying to get an even more readable version by sorting the subregions by population size before rendering the diagram, doesn't yield the same result we saw when using Gadfly or VegaLite:
"""

# ╔═╡ b4ba5860-8e93-11eb-3a81-0bb1bd36aca2
subregions_cum_sorted = sort(subregions_cum, :Pop2019)

# ╔═╡ a151f124-8ea8-4ffc-bc67-3f0b41701df0
@df subregions_cum_sorted bar(:Subregion, :Pop2019, group = :Region,
	orientation = :hor,
	xlabel = "Population [millions]", 
	ylabel = "Subregion",
	yticks = :all,
...
# ╔═╡ 639a59d1-d479-46a5-9479-ea38701d078c
md"""
The reason for the difference in ordering the subregions (as compared to Gadfly/VegeLite) is the grouping mechanism of Plots. As Plots doesn't know the concept of graphic dimensions, we used the grouping mechanism instead (as a workaround) to display the regions in different colours. But as the name suggests, this mechanism groups the data indeed. In contrast, applying an additional graphic dimension leaves the structure of the original data intact and merely applies a graphic attribute to it during the rendering process.
"""
...
# ╔═╡ 69260de5-f87f-4032-ae97-738d5eae0ad0
@df countries scatter(:Pop2019, :PopChangePct, group = :Region)
...
# ╔═╡ 03c36097-fb20-41da-90cf-2606cf146cbf
@df countries scatter(:Pop2019, :PopChangePct, group = :Region,
	title = "Population vs Growth Rate, 2019",
	xscale = :log10,
	xlabel = "Population [millions]",
	xticks = ([1, 100, 1000], [1, 100, 1000]),
	ylabel = "Growth Rate [%]",
	background_color = "ghostwhite")
...
# ╔═╡ 1fc7124b-a74e-493c-8be0-f74a05f71ab6
@df countries histogram(:GDPperCapita)

# ╔═╡ e54c01a2-fcfe-48ba-89c8-471dfc6a42ed
md"""
And again we add labels etc. The number of bins can be changed by using the attribute `bins`.
"""

# ╔═╡ d3cec123-b029-44c7-8550-2c5a6ef33509
@df countries histogram(:GDPperCapita,
	bins = 10,
	...
	background_color = "ghostwhite",
	xticks = (0:50000:200000, 0:50000:200000),
	legend = false)
...
# ╔═╡ 2490667a-0d9f-4dc9-94a5-04fc309ad6b2
@df countries boxplot(:Region, :GDPperCapita, group = :Region,
	legend = false,
...
@df countries violin(:Region, :GDPperCapita, group = :Region,
	legend = false,
	title = "GDP per Capita by Region, 2019",
	xlabel = "Region",
	ylabel = "GDP per Capita [USD]",
...
md"""
*Note*: The GDP values on the y-axis in this diagram are restricted to values <= 100.000 (in order to focus on the "interesting" parts of the diagram). To get the same result in the box plot above, the attribute `ylims` can be set there as well.
"""
